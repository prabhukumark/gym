...
<body>
    ...
    <form id="signupForm" action="r.php" method="post">
        <h2>Sign Up</h2>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $checkUsernameQuery = "SELECT * FROM users WHERE username = '$inputUsername' or email='$inputEmail'";
    $result = $conn->query($checkUsernameQuery);
   
    $checkUsernameQuery2 = "SELECT * FROM users WHERE email='$inputEmail'";
        
    $result2 = $conn->query($checkUsernameQuery2);
    $checkUsernameQuery3 = "SELECT * FROM users WHERE username = '$inputUsername' and email='$inputEmail'";
    ...
    if ($result->num_rows > 0) {
        ...
        $errorMessage = "!!!**please enter username which has atleast one alphabet letter**!!!";
    ...
    elseif (preg_match($passwordRegex, $inputPassword)) {
        if ($inputPassword !== $inputConfirmPassword) {
            ...
            $insertQuery = "INSERT INTO users (username, name, email, password) VALUES ('$inputUsername', '$inputLastName', '$inputEmail', '$hashedPassword')";
